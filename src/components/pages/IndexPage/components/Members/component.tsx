...
export const Component: FC<Props> = ({ members }) => {
  const [selectedMember, setSelectedMember] = useState<
    Queries.TopQuery['members']['nodes'][0] | null
  >(null);
    ...
    <div id={'members'}>
      ...
      <Wrapper sx={{ pt: 4 }}>
        ...
        <Members sx={{ flexGrow: 1 }}>
          {members.map((member) => {
            const img = member.picture?.gatsbyImageData
              ? getImage(member.picture.gatsbyImageData)
              : undefined;
            ...
            <DialogContent sx={{ p: 6 }}>
              <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2 }}>
                <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  <Typography variant={'body1'} sx={{ whiteSpace: 'nowrap', mb: 2 }}>
                    {selectedMember.nameEn}
                  </Typography>
                  <Box sx={{ width: 200 }}>
                    {selectedMemberImage && (
                      <GatsbyImage image={selectedMemberImage} alt={selectedMember.name ?? ''} />
                    )}
                  </Box>
                </Box>
                <Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 4 }}>
                ...
                <Favorites>
                  <dt>
                    <Typography variant={'caption'}>好きな温泉</Typography>
                  </dt>
                  <dd>
                    <Typography variant={'caption'}>{selectedMember.favoriteOnsenArea}</Typography>
                  </dd>
                  <dt>
                    <Typography variant={'caption'}>好きな泉質</Typography>
                  </dt>
                  <dd>
                    <Typography variant={'caption'}>{selectedMember.favoriteSpring}</Typography>
                  </dd>
                </Favorites>
              </Box>
            </DialogContent>
