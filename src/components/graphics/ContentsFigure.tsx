/* eslint-disable react/jsx-curly-brace-presence */
import type { FC } from 'react';
import React from 'react';

const ContentsFigure: FC = () => (
  <svg
    id="Group_102"
    data-name="Group 102"
    xmlns="http://www.w3.org/2000/svg"
    width="149.024"
    height="122.756"
    viewBox="0 0 149.024 122.756"
  >
    <defs>
      <clipPath id="clip-path">
        <rect
          id="Rectangle_114"
          data-name="Rectangle 114"
          width="149.024"
          height="122.756"
          fill="none"
        />
      </clipPath>
    </defs>
    <rect
      id="Rectangle_112"
      data-name="Rectangle 112"
      width="46.826"
      height="69.668"
      transform="translate(1.417 1.418)"
      fill="#f0908d"
    />
    <g id="Group_91" data-name="Group 91">
      <g id="Group_90" data-name="Group 90" clipPath="url(#clip-path)">
        <rect
          id="Rectangle_113"
          data-name="Rectangle 113"
          width="46.826"
          height="69.668"
          transform="translate(1.417 1.418)"
          fill="none"
          stroke="#000"
          strokeWidth="2.835"
        />
      </g>
    </g>
    <rect
      id="Rectangle_115"
      data-name="Rectangle 115"
      width="37.689"
      height="34.263"
      transform="translate(109.917 1.418)"
      fill="#fff"
    />
    <g id="Group_93" data-name="Group 93">
      <g id="Group_92" data-name="Group 92" clipPath="url(#clip-path)">
        <rect
          id="Rectangle_116"
          data-name="Rectangle 116"
          width="37.689"
          height="34.263"
          transform="translate(109.917 1.418)"
          fill="none"
          stroke="#000"
          strokeWidth="2.835"
        />
      </g>
    </g>
    <rect
      id="Rectangle_118"
      data-name="Rectangle 118"
      width="37.689"
      height="85.658"
      transform="translate(109.917 35.681)"
      fill="#e94709"
    />
    <g id="Group_95" data-name="Group 95">
      <g id="Group_94" data-name="Group 94" clipPath="url(#clip-path)">
        <rect
          id="Rectangle_119"
          data-name="Rectangle 119"
          width="37.689"
          height="85.658"
          transform="translate(109.917 35.681)"
          fill="none"
          stroke="#000"
          strokeWidth="2.835"
        />
      </g>
    </g>
    <rect
      id="Rectangle_121"
      data-name="Rectangle 121"
      width="61.674"
      height="35.405"
      transform="translate(48.244 1.418)"
      fill="#fff"
    />
    <g id="Group_97" data-name="Group 97">
      <g id="Group_96" data-name="Group 96" clipPath="url(#clip-path)">
        <rect
          id="Rectangle_122"
          data-name="Rectangle 122"
          width="61.674"
          height="35.405"
          transform="translate(48.244 1.418)"
          fill="none"
          stroke="#000"
          strokeWidth="2.835"
        />
      </g>
    </g>
    <rect
      id="Rectangle_124"
      data-name="Rectangle 124"
      width="108.5"
      height="35.405"
      transform="translate(1.417 71.086)"
      fill="#fff"
    />
    <g id="Group_99" data-name="Group 99">
      <g id="Group_98" data-name="Group 98" clipPath="url(#clip-path)">
        <rect
          id="Rectangle_125"
          data-name="Rectangle 125"
          width="108.5"
          height="35.405"
          transform="translate(1.417 71.086)"
          fill="none"
          stroke="#000"
          strokeWidth="2.835"
        />
      </g>
    </g>
    <rect
      id="Rectangle_127"
      data-name="Rectangle 127"
      width="61.674"
      height="35.405"
      transform="translate(48.244 35.681)"
      fill="#fff"
    />
    <rect
      id="Rectangle_128"
      data-name="Rectangle 128"
      width="61.674"
      height="35.405"
      transform="translate(48.244 35.681)"
      fill="none"
      stroke="#000"
      strokeWidth="2.835"
    />
    <rect
      id="Rectangle_129"
      data-name="Rectangle 129"
      width="108.5"
      height="35.405"
      transform="translate(1.417 85.933)"
      fill="#fff"
    />
    <g id="Group_101" data-name="Group 101">
      <g id="Group_100" data-name="Group 100" clipPath="url(#clip-path)">
        <rect
          id="Rectangle_130"
          data-name="Rectangle 130"
          width="108.5"
          height="35.405"
          transform="translate(1.417 85.933)"
          fill="none"
          stroke="#000"
          strokeWidth="2.835"
        />
      </g>
    </g>
  </svg>
);

export default ContentsFigure;
